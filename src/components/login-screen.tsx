...
import { Loader2, Database, Sparkles, ShieldCheck, FileText, Lock, Server, Eye, Ban } from 'lucide-react'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function LoginScreen() {
  ...
  const handleSeed = async () => {
    ...
    setSeedMessage(null)
    setError(null)
    const res = await api.post<{ message: string; created?: string[]; skipped?: string[] }>('/auth/seed', {})
    if (res.error) {
      setError(res.error)
    } else if (res.data) {
      if (res.data.created && res.data.created.length > 0) {
        setSeedMessage(`Demo users created! You can now log in with any role.`)
      } else {
        setSeedMessage('All demo users already exist. You can log in directly.')
      }
    }
    setIsSeeding(false)
  }
  ...
  const handleAutoSeedAndLogin = async () => {
    setIsLoading(true)
    setError(null)
    setSeedMessage(null)

    // Step 1: Seed demo users (idempotent - creates missing ones)
    setIsSeeding(true)
    const seedRes = await api.post<{ message: string; created?: string[] }>('/auth/seed', {})
    setIsSeeding(false)
    ...
    if (seedRes.data?.created && seedRes.data.created.length > 0) {
      setSeedMessage('Demo users created!')
    } else {
      setSeedMessage('Using existing users...')
    }

    // Step 2: Login as admin
    const loginRes = await api.post<{ user: { id: string; name: string; email: string; role: string; departmentId?: string }; mustChangePassword?: boolean }>('/auth', {
    ...
    if (loginRes.error) {
      ...
      login(loginRes.data.user, loginRes.data.mustChangePassword ?? false)
    }
    setIsLoading(false)
  }
          ...
          <CardContent className="grid p-0 md:grid-cols-2">
            ...
            <form onSubmit={handleLogin} className="p-6 md:p-8">
              <FieldGroup>
                ...
                {/* Seed message */}
                {seedMessage && (
                  <div className="px-4 py-2.5 rounded-lg bg-[#10B981]/10 border border-[#10B981]/20 text-[#10B981] text-sm text-center">
                    {seedMessage}
                  </div>
                )}
                ...
                <Field className="grid grid-cols-2 gap-4">
                  <Button
                    variant="outline"
                    type="button"
                    onClick={handleAutoSeedAndLogin}
                    disabled={isLoading || isSeeding}
                    className="border-[#10B981]/30 text-[#10B981] hover:bg-[#10B981]/10 hover:border-[#10B981]/50"
                  >
                    {isSeeding ? (
                      <>
                        <Loader2 className="size-4 animate-spin" />
