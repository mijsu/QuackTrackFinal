...
function SystemTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-2xl p-6">
        <h2 className="font-heading font-bold text-foreground text-lg mb-4">Schedule Generation Defaults</h2>
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label className="text-muted-foreground text-xs">Default Time Slot Duration (hrs)</Label>
              <Input
                defaultValue="1.5"
                type="number"
                step="0.5"
            ...
            <div className="space-y-1">
              <Label className="text-muted-foreground text-xs">Max Conflict Severity Allowed</Label>
              <Select defaultValue="warning">
                <SelectTrigger className="w-full bg-secondary border-border text-foreground">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-popover border-border">
                  <SelectItem value="error" className="text-accent-foreground focus:bg-accent focus:text-accent-foreground">Error Only</SelectItem>
                  <SelectItem value="warning" className="text-accent-foreground focus:bg-accent focus:text-accent-foreground">Warnings Allowed</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
      ...
      <div className="bg-card border border-red-500/20 rounded-2xl p-6">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h2 className="font-heading font-bold text-foreground text-lg mb-1">Re-seed Database</h2>
            ...
            {!showSeedConfirm ? (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500/10 text-red-400 border border-red-500/20 text-sm hover:bg-red-500/20 transition-colors"
              >
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500 text-white text-sm hover:bg-red-600 transition-colors disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-secondary/50 text-muted-foreground text-sm hover:text-foreground transition-colors"
                >
                  Cancel
                </button>
              </div>
            ...
            {seedMutation.isSuccess && (
              <p className="text-green-400 text-sm mt-3">Database re-seeded successfully!</p>
            )}
            {seedMutation.isError && (
              <p className="text-red-400 text-sm mt-3">Failed to re-seed database.</p>
            )}
          </div>
        </div>
      </div>
...
function GenerationTab() {
  ...
  const createConfigMutation = useMutation({
    mutationFn: async () => {
      return api.post('/generation-configs', {
        name: newConfig.name,
  ...
  const updateConfigMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Record<string, unknown> }) => {
      return api.put(`/generation-configs/${id}`, data)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['generation-configs'] })
  ...
  const deleteConfigMutation = useMutation({
    mutationFn: async (id: string) => {
      return api.delete(`/generation-configs/${id}`)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['generation-configs'] })
    },
  ...
  const setDefaultMutation = useMutation({
    mutationFn: async (id: string) => {
      return api.put(`/generation-configs/${id}`, { isDefault: true })
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['generation-configs'] })
    },
