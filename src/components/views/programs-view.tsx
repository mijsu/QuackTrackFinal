...
const emptyForm = { name: '', code: '', description: '', departmentId: '' }
...
export function ProgramsView() {
  ...
  const createMut = useMutation({
    mutationFn: (body: unknown) => api.post('/programs', body),
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, ...body }: Record<string, unknown>) => api.put(`/programs/${id}`, body),
    onSuccess: () => { setError(null); qc.invalidateQueries({ queryKey: ['programs'] }); closeDialog() },
