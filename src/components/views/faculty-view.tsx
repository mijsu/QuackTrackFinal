...
function facultyTypeLabel(t: string | null) {
  if (!t) return '—'
  const labels: Record<string, string> = { regular: 'Regular', masteral: 'Masteral' }
...
export function FacultyView() {
  ...
  const { data: facultyData, isLoading } = useQuery({
    queryKey: ['faculty', search, filterDept, filterType, filterStatus],
    queryFn: () => {
      const params = new URLSearchParams()
      params.set('role', 'faculty')
      if (search) params.set('search', search)
      if (filterDept !== 'all') params.set('departmentId', filterDept)
      if (filterType !== 'all') params.set('facultyType', filterType)
  ...
  const createMut = useMutation({
    mutationFn: (body: unknown) => api.post('/users', { ...body, role: 'faculty' }),
    onSuccess: (res) => {
      setError(null)
      qc.invalidateQueries({ queryKey: ['faculty'] })
      // Show credentials modal instead of closing the dialog
      if (res?.data?._tempCredentials) {
        setCredentialsData({
          email: res.data._tempCredentials.email,
          tempPassword: res.data._tempCredentials.tempPassword,
          personalEmail: res.data._tempCredentials.personalEmail || undefined,
          emailSent: res.data._tempCredentials.emailSent,
        })
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, ...body }: Record<string, unknown>) => api.put(`/users/${id}`, body),
              ...
              <TableBody>
                {faculty.map(f => (
                  ...
                  >
                    ...
                    <TableCell className="text-muted-foreground">{facultyTypeLabel(f.facultyType)}</TableCell>
            ...
            <div className="space-y-4">
              ...
              {workload && workload.schedules.length > 0 && (
                <div className="bg-muted rounded-xl p-4 space-y-2">
                  <h3 className="font-heading font-bold text-foreground text-sm">Current Schedules ({workload.schedules.length})</h3>
                  <div className="max-h-64 overflow-y-auto custom-scrollbar space-y-2">
                    {workload.schedules.map(s => (
