...
export async function GET(request: NextRequest) {
  try {
    ...
    const dayHdrH = 20
    const cardH = 48  // height per class card
    ...
    // If grid doesn't fit, reduce card height proportionally
    const effectiveCardH = gridH > availableH
      ? Math.max(30, Math.floor((availableH - dayHdrH) / Math.max(1, Math.max(...DAYS.map(d => byDay[d].length)))))
      : cardH
    ...
    for (let di = 0; di < DAYS.length; di++) {
      ...
      if (isEmpty) {
        ...
        for (let si = 0; si < daySchedules.length; si++) {
          ...
          const tx = colX + 9
          const tw = colW - 13
          const scale = effectiveCardH / 48 // scale factor if cards are compressed

          // Time
          doc.font('Helvetica').fontSize(Math.max(5.5, 7 * scale)).fillColor(C.gray)
          text(doc, `${formatTime12(s.startTime)} – ${formatTime12(s.endTime)}`, tx, by + 1, { width: tw })

          // Subject code
          doc.font('Helvetica-Bold').fontSize(Math.max(6.5, 8.5 * scale)).fillColor(C.dark)
          text(doc, esc(s.subject?.subjectCode || '—'), tx, by + 1 + 9 * scale, { width: tw })

          // Subject name
          doc.font('Helvetica').fontSize(Math.max(5.5, 7 * scale)).fillColor(C.gray)
          text(doc, esc(s.subject?.subjectName || '—'), tx, by + 1 + 18 * scale, { width: tw })

          // Section
          doc.font('Helvetica').fontSize(Math.max(5, 6.5 * scale)).fillColor(C.grayLight)
          text(doc, esc(s.section?.sectionName || '—'), tx, by + 1 + 26 * scale, { width: tw })

          // Type label
          doc.font('Helvetica-Bold').fontSize(Math.max(4.5, 6 * scale)).fillColor(typeColor)
          text(doc, typeLabel, tx, by + 1 + 33 * scale, { width: tw })
