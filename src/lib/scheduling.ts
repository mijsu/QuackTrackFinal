...
interface GenerationInput {
  ...
  departmentId?: string
  configId?: string
  classType?: string // 'regular', 'executive', 'masteral', or undefined (all)
}
 ...
 * 4. Section spread — avoid clustering all section classes on same day
 */
function scoreSlot(
  day: string,
  slot: { start: string; end: string },
...
export async function generateSchedules(input: GenerationInput): Promise<GenerationResult> {
  ...
  try {
    ...
    // Filter by classType if specified — this only affects which faculty are eligible
    // "masteral" is a faculty type (faculty with master's degrees), NOT a class type
    // Masteral faculty can teach BOTH regular and executive classes
    // Regular faculty can ONLY teach regular classes
    if (classType === 'regular') {
      // Regular classes: both regular and masteral faculty are eligible
      facultyWhere.facultyType = { in: ['regular', 'masteral'] }
    } else if (classType === 'executive') {
      // Executive classes: only masteral faculty are eligible
    ...
    // classType (regular / executive / masteral) only filters which FACULTY are eligible.
    // Executive classes use masteral faculty (those with master's degrees).
    // Sections and subjects are the same regardless of class type — the distinction
    // is about which faculty pool can be assigned, not about separate programs.
    const filteredSubjects = subjects

    console.log(`📊 Data found: ${filteredSubjects.length} subjects, ${faculty.length} faculty, ${sections.length} sections (classType=${classType || 'all'})`)

    if (filteredSubjects.length === 0 || faculty.length === 0 || sections.length === 0) {
      const missing: string[] = []
      if (filteredSubjects.length === 0) missing.push(`subjects (semester="${semester}"${classType ? `, type="${classType}"` : ''})`)
      if (faculty.length === 0) missing.push(`faculty${classType ? ` (type="${classType}")` : ''}`)
      if (sections.length === 0) missing.push(`sections (semester="${semester}"${classType ? `, type="${classType}"` : ''})`)
      ...
      return {
        ...
        score: 0,
        status: 'failed',
        message: `Insufficient data: missing ${missing.join(', ')}. Need subjects, faculty, and sections for the selected semester.`,
      }
    }
    ...
    for (const section of sections) {
      const sectionSubjects = filteredSubjects.filter(
        s => s.programId === section.programId && s.yearLevel === section.yearLevel
      )
    ...
    for (const task of shuffled) {
      ...
      // Pick the best scored multi-slot placement
      ...
      // Fallback: if no scored candidate, try first available without scoring
      if (!bestCandidate) {
        for (const f of sortedFaculty) {
          const facultyBooked = facultySlotBooked.get(f.id)!
          const sectionBooked = sectionSlotBooked.get(task.sectionId)!
...
export async function detectConflicts(scheduleVersionId: string) {
  ...
  for (const s of schedules) {
    const existing = facultyTotalUnits.get(s.facultyId)
    const units = s.subject.units
    if (existing) {
      existing.total += units
    } else {
      facultyTotalUnits.set(s.facultyId, {
        name: s.faculty.name,
        total: units,
